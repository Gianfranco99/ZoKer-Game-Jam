...
mod tests {
    ...
    use dojo_starter::models::{Match, m_Match, ProposalMatch,m_ProposalMatch};

    fn namespace_def() -> NamespaceDef {
        let ndef = NamespaceDef {
            namespace: "dojo_starter", resources: [
                TestResource::Model(m_Match::TEST_CLASS_HASH.try_into().unwrap()),
                TestResource::Model(m_ProposalMatch::TEST_CLASS_HASH.try_into().unwrap()),
    ...
    fn test_world_test_set() {
        ...
        // let mut match_id:Match = world.read_model(1_u64);
        // println!("match {}" ,     match_id.id);
        // let mut position: Position = world.read_model(caller);
        // assert(match_id == 42_u64, 'initial position wrong');
